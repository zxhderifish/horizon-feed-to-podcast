...
def main():
    ...
    parser.add_argument("--fetch-only", action="store_true", help="Only fetch + URL-dedup, no AI/output")
    parser.add_argument("--json", action="store_true", help="Emit fetched items as JSON on stdout")
    ...
    try:
        ...
        if args.fetch_only:
            ...
            from .fetch_only import collect_fetch_only

            # Keep stdout clean for JSON; route progress to stderr.
            orchestrator.console = _Console(stderr=True)
            items = asyncio.run(collect_fetch_only(orchestrator, args.hours))
            if args.json:
